...
["", "sudoku.dart", , Q, {
  ...
  main: [function() {
    ...
    $.tbody = J.createTBody$0$x($.board);
    Q.makeSudokuBoard();
    Q.ChangeCurrentCell();
    t1 = H.setRuntimeTypeInfo(new W._EventStream(window, C.EventStreamProvider_keydown._eventType, false), [null]);
    H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(Q.mykeyDown$closure()), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
    t1 = H.setRuntimeTypeInfo(new W._EventStream(window, C.EventStreamProvider_keyup._eventType, false), [null]);
    H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(Q.callCheckBoard$closure()), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
    t1 = H.setRuntimeTypeInfo(new W._EventStream(window, C.EventStreamProvider_click._eventType, false), [null]);
    H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(Q.clickDown$closure()), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
  }, "call$0", "main$closure", 0, 0, 1],
  clickDown: [function(e) {
    Q.CheckCurrentCell();
    Q.ChangeCurrentCell();
    Q.CheckBoard();
  }, "call$1", "clickDown$closure", 2, 0, 6],
  mykeyDown: [function(e) {
    if (!!J.getInterceptor(e).$isKeyboardEvent)
      switch (e.keyCode) {
        case 38:
          $.currentRow = $.currentRow - 1;
          for (; Q.CheckNextCell() !== true;)
            $.currentRow = $.currentRow - 1;
          Q.ChangeCurrentCell();
          Q.CheckBoard();
          break;
  ...
  }, "call$1", "mykeyDown$closure", 2, 0, 6],
  callCheckBoard: [function(e) {
    Q.CheckBoard();
  }, "call$1", "callCheckBoard$closure", 2, 0, 6],
  CheckBoard: function() {
    var worked, current, c, exception, t1, i, i0, t2, t3, boxRowOffset, boxColOffset, k, m, t4, check, j;
    worked = true;
    c = J.get$cells$x(J.$index$asx(J.get$rows$x($.board), $.currentRow));
    current = $.tableCell.textContent;
    try {
      H.Primitives_parseInt(current, null, null);
    } catch (exception) {
      H.unwrapException(exception);
      worked = false;
    }
    ...
    if (J.$eq(current, ""))
      ...
      J.set$background$x(J.get$style$x($.tableCell), "#f44");
      return;
    } else
      for (t1 = c._list, i = 0; i < t1.length; ++i) {
        if ($.currentCell === i)
          ;
        else if (J.$eq(current, t1[i].textContent)) {
          J.set$background$x(J.get$style$x($.tableCell), "#f44");
          return;
        }
        for (i0 = 0; i0 < J.get$length$asx(J.get$rows$x($.board)); ++i0)
          if (i0 === $.currentRow)
            ;
          else {
            t2 = J.get$cells$x(J.$index$asx(J.get$rows$x($.board), i0));
            t3 = $.currentCell;
            t2 = t2._list;
            if (t3 < 0 || t3 >= t2.length)
              return H.ioore(t2, t3);
            t3 = t2[t3].textContent;
            t2 = current;
        ...
        boxRowOffset = C.JSInt_methods._tdivFast$1($.currentRow, 3) * 3;
        boxColOffset = C.JSInt_methods._tdivFast$1($.currentCell, 3) * 3;
        for (k = 0; k < 3; ++k)
          for (t2 = boxRowOffset + k, m = 0; m < 3; ++m) {
            t3 = boxColOffset + m;
            if ($.currentCell === t3 && $.currentRow === t2)
              ;
            else {
              t4 = J.get$cells$x(J.$index$asx(J.get$rows$x($.board), t2))._list;
              if (t3 < 0 || t3 >= t4.length)
                return H.ioore(t4, t3);
              t3 = t4[t3].textContent;
              t4 = current;
    ...
    for (check = 0, i = 0; i < J.get$length$asx(J.get$rows$x($.board)); ++i) {
      for (j = 0; j < J.get$cells$x(J.$index$asx(J.get$rows$x($.board), i))._list.length; ++j) {
        t1 = J.get$cells$x(J.$index$asx(J.get$rows$x($.board), i))._list;
        if (j >= t1.length)
          return H.ioore(t1, j);
        if (t1[j].textContent !== "")
          ++check;
      }
      J.set$background$x(J.get$style$x($.tableCell), "");
      if (check === 81)
        document.querySelector("#pageTitle").textContent = "winner";
    }
  },
  CheckCurrentCell: function() {
    var r, t1, i, t2, j;
    r = J.get$rows$x($.board);
    for (t1 = J.getInterceptor$asx(r), i = 0; i < t1.get$length(r); ++i)
      for (t2 = J.get$cells$x(t1.$index(r, i))._list, j = 0; j < t2.length; ++j)
        if (t2[j] === document.querySelector(":focus")) {
          $.currentRow = i;
          $.currentCell = j;
        }
  },
  CheckNextCell: function() {
    var t1, t2;
    t1 = $.currentRow;
    if (t1 === -1) {
      $.currentRow = 8;
    ...
    t1 = J.get$cells$x(J.$index$asx(J.get$rows$x($.board), t1));
    t2 = $.currentCell;
    t1 = t1._list;
    ...
    t2 = t1[t2];
    $.tableCell = t2;
    return J.get$isContentEditable$x(t2);
  },
  ...
  makeSudokuBoard: function() {
    var i, rows, t1, j, t2, t3, puzzleplace;
    for (i = 0; i < 9; ++i) {
      rows = J.addRow$0$x($.tbody);
      for (t1 = J.getInterceptor$x(rows), j = 0; j < 9; ++j) {
        t2 = $.puzzle;
        t3 = $.counter;
        puzzleplace = C.JSString_methods.substring$2(t2, t3, t3 + 1);
        if (puzzleplace === "0") {
          t1.insertCell$1(rows, j).textContent = "";
          t2 = rows.cells;
          new W._WrappedList(t2).$builtinTypeInfo = [W.TableCellElement];
          if (j >= t2.length)
            return H.ioore(t2, j);
          J.setAttribute$2$x(t2[j], "contenteditable", "true");
        } else {
          t1.insertCell$1(rows, j).textContent = puzzleplace;
          t2 = rows.cells;
          new W._WrappedList(t2).$builtinTypeInfo = [W.TableCellElement];
          if (j >= t2.length)
            return H.ioore(t2, j);
          J.setAttribute$2$x(t2[j], "contenteditable", "false");
          t2 = rows.cells;
          new W._WrappedList(t2).$builtinTypeInfo = [W.TableCellElement];
          if (j >= t2.length)
            return H.ioore(t2, j);
          J.get$classes$x(t2[j]).add$1(0, "default");
        }
        t2 = t1.get$_cells(rows);
        new W._WrappedList(t2).$builtinTypeInfo = [W.TableCellElement];
        if (j >= t2.length)
          return H.ioore(t2, j);
        J.setAttribute$2$x(t2[j], "onkeypress", "return (this.innerText.length <= 0)");
        $.counter = $.counter + 1;
      }
    }
...
J.get$isContentEditable$x = function(receiver) {
  return J.getInterceptor$x(receiver).get$isContentEditable(receiver);
};
...
$.currentRow = 0;
$.currentCell = 0;
$.puzzle = "542731869187296345936485127213549678869127453475368912351974286694852731728613590";
$.counter = 0;
...
function dart_precompiled($collectedClasses) {
  ...
  Element.prototype.get$isContentEditable = function(receiver) {
  ...
  function MappedIterator(_current, __internal$_iterator, _f) {
    this._current = _current;
